...
/* ------------------------------------------------------------------------------ */
		...
		function main() {
			...
			// Load data
			$.get('php/list.php'          ).done( (d) => docList(d)        ).fail( () => alert('No document to show') );
 			$.get('assets/categories.json').done( (d) => categoriesList(d) ).fail( () => alert('No category loaded') );
...
/* ------------------------------------------------------------------------------ */
		...
		function downloadDoc(type) {
			...
			if (type=='html') {
				...
				saveAsXML(currentFilename, $('#file')[0], 'assets/TEI.xsl', options)
			} else {
